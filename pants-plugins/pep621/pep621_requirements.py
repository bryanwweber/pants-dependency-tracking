...
def parse_pyproject_toml(
    ...
    *,
    rel_path: str,
    overrides: MutableMapping[NormalizedName, MutableMapping[str, Any]],
    ...
    if not deps_vals and not optional_dependencies:
        raise KeyError(
            softwrap(
                "No section `project.dependencies` or "
                f"`project.optional-dependencies` found in {rel_path}"
            )
        )
...
class PEP621RequirementsTargetGenerator(TargetGenerator):
    ...
    help = (
        "Generate a `python_requirement` for each entry in a PEP 621 compliant "
        "pyproject.toml."
    )
    ...
    digest_contents = await Get(
        ...
        PathGlobs(
            ...
            description_of_origin=(
                f"{generator}'s field `{PEP621RequirementsSourceField.alias}`"
            ),
        ),
    )
    ...
    def generate_tgt(
        ...
        if Dependencies.alias in tgt_overrides:
            tgt_overrides[Dependencies.alias] = list(
                tgt_overrides[Dependencies.alias]
        ...
        return PythonRequirementTarget(
            {
                **request.template,
                ...
                PythonRequirementModulesField.alias: module_mapping.get(
