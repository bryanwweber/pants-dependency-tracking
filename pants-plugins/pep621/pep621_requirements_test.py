...
def assert_python_requirements(
    ...
    *,
    ...
    rule_runner.write_files(
        {"BUILD": build_file_entry, pyproject_toml_relpath: pyproject_toml}
    )
...
def test_multiple_versions(rule_runner: RuleRunner) -> None:
    """This tests that we correctly create a new python_requirement for each unique
    dependency name in a pyproject.toml file, grouping duplicated dependency names to
    handle multiple requirement strings per PEP 508."""
    file_addr = Address("", target_name="reqs", relative_file_path="pyproject.toml")
    assert_python_requirements(
        rule_runner,
...
def test_invalid_req(rule_runner: RuleRunner) -> None:
    ...
    with engine_error(
        contains=("Invalid requirement 'Not A Valid Req == 3.7' in pyproject.toml")
    ):
        assert_python_requirements(
            rule_runner,
            "pep621_requirements(name='reqs')",
...
def test_source_override(rule_runner: RuleRunner) -> None:
    file_addr = Address(
        "", target_name="reqs", relative_file_path="subdir/pyproject.toml"
    )
    assert_python_requirements(
        rule_runner,
        ...
        expected_targets={
            ...
            TargetGeneratorSourcesHelperTarget(
                {"source": "subdir/pyproject.toml"}, file_addr
            ),
        },
    )
